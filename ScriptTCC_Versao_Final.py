...
volume_xgboost = comparacao_2025["Previsao_XGBoost"].sum()
...
# Heatmap de Diferença Percentual por Categoria
...
comp_cat["Empresa x Real"] = (comp_cat["Qtd_Prevista_Empresa"] - comp_cat["Qtd_Real"]) / comp_cat["Qtd_Real"].replace(0, 1)
comp_cat["XGBoost x Real"] = (comp_cat["Previsao_XGBoost"] - comp_cat["Qtd_Real"]) / comp_cat["Qtd_Real"].replace(0, 1)

df_heatmap = comp_cat.set_index("Categoria")[["Empresa x Real", "XGBoost x Real"]]
limite_max = max(abs(df_heatmap.values.min()), abs(df_heatmap.values.max()))

fig, ax = plt.subplots(figsize=(8, len(df_heatmap) * 0.4 + 2))
...
sns.heatmap(df_heatmap, 
            ...
            linewidths=1,             
            linecolor='white',       
            cbar_kws={"orientation": "horizontal", "pad": 0.08, "shrink": 0.6},
            ax=ax)
...
plt.tight_layout()
salvar_fig("16_heatmap_diferenca_categoria.png")
plt.show()
